...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/23 19:32:07 by efinicke          #+#    #+#             */
/*   Updated: 2024/07/23 19:32:08 by efinicke         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    invite(Command &command, Server &server)
{
    ...
    if (channel->findUser(command.getClientSfd(), channel->getUsersMap()) == NULL)                                                          // le sender ne peut pas inviter qq1 car il n'est pas sur le chan
        ...
        server.sendMsg(ERR_NOSUCHCHANNEL(sender->getLocalhost(), sender->getNickname(), channelName), command.getClientSfd());
